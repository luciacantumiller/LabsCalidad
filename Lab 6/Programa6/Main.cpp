...
int main() {
	...
	if (myfile.is_open()) {
		if (myfile.peek() != std::ifstream::traits_type::eof()) {
			...
			double dof = n - 2.0; 
			long double x = (abs(r)*sqrt(dof))/sqrt(1 - pow(r,2)); 
			double sig = 1 - 2*sim.getSimpson(x, dof);
			double ran = helper.GetX(0.35, dof) * helper.StandardDeviation(xValues, yValues, bZero, bOne, n)*sqrt(1.0 + (1.0/n)+(pow(xk - xAvg,2))/(helper.SumTwoNumbersSqrd(xValues, n, xAvg)));
